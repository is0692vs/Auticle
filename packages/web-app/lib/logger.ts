...
export const logger = {
  ...
  success: (message: string, ...args: unknown[]) => {
    console.log(`%c${LOG_PREFIX} [SUCCESS]`, LOG_STYLES.success, message, ...args);
  },
  ...
  error: (message: string, ...args: unknown[]) => {
    console.error(`%c${LOG_PREFIX} [ERROR]`, LOG_STYLES.error, message, ...args);
  },
  ...
  cache: (action: string, key: string) => {
    console.log(`%c${LOG_PREFIX} [CACHE]`, LOG_STYLES.data, `${action}: ${key}`);
  },
};
